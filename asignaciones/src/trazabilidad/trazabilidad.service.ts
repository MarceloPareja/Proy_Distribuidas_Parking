...
export class TrazabilidadService {
   ...
   *   → El interceptor llama a este método con `.catch()` (fire-and-forget).
   *   → Pero internamente el método sí es async para poder loguear errores.
   *
   * ¿Cómo se asegura que un fallo aquí no rompa la respuesta?
   *   → try/catch interno: cualquier excepción se atrapa y se loguea.
   *   → El interceptor además no espera la resolución de esta promesa.
   */
